...
public class AmazonMusicService : DataService
{
    ...
    public override async Task<SearchResult> GetSearchResultAsyncInternal(string artistName)
    {
        ...
        foreach (var entity in parsed)
        {
            ...
            {
                result.Singles!.Add(new Entity(entity.Title.Substring(0, entity.Title.Length - 9), int.Parse(entity.Year!)));
            }
            else
            {
                result.Others!.Add(new Entity(entity.Title, int.Parse(entity.Year!)));
            }
        }
